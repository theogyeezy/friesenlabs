...
| ✅ **Live (DB)** | FLEETAGENT schema migration: `workspace_keys`/`leads`/`playbooks`/`predictions` + `tenant_settings` ALTERs + grants | ran live via `uplift-migrate-oneoff:2` (exit 0), **live isolation test PASS** | api/cube image roll (paused at the approval gate — see TODO) |
| 🟙 **Merged, awaiting roll** | Revenue path real (checkout URL surfaced, atomic settlement + webhook field verification, workspace-key pool→Secrets Manager, Cognito login fix, `/public/leads`, @friesenlabs.com test-bypass); accountability (kill switch / traces / autonomy persisted + routed); data-plane (Cube RLS GUC fix #177, worker cube client, citations) | 22 PRs merged green on `main` `20384e9` | rebuild+roll provisioning-Lambda + cube images, then approve the Deploy apply (TODO) |
| 🟙 **Merged, awaiting roll** | MVP features: **Balto** NL view-creation in chat, **Agent Studio** + 5 playbooks, **connectors** (CSV/GoHighLevel/Stripe-read), **dashboards v2**, **Cortex depth** (loader/retrain/signed artifacts/drift), demo-tenant + knowledge-corpus seed | branches `feat/mvp-*` preserved on origin for further dev | image roll + (Cortex) live S3 registry + seeded corpus |
