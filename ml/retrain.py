...
SNS drift paging
----------------
`run_scheduled_retrain` only COMPUTES the drift verdict (returned in `result["drift"]`); it does
NOT page. The single drift-publish surface is the fan-out driver scripts/ml/retrain_all.py, which
hands each tenant's verdict to `ml.drift_alert.DriftNotifier.notify` (one publish per drifting
tenant). Keeping the publish out of this orchestrator avoids a double-page when the fan-out runs.
"""
