...
- **Status:** IN-PROGRESS (Nick) @feat/nick-req-006-008 — POSTHOG_PROJECT_KEY_VALUE folded into the EXISTING api_signup_env gate (deviation: zero api rolls now; it lands with the signup go-live roll) + provisioning Lambda env (in-place update; platform secret verified to hold a value); execution role gains the exact posthog ARN; POSTHOG_HOST plain var (only-if-set).
...
- **Status:** IN-PROGRESS (Nick) @feat/nick-req-006-008 — api task-role policy `connector-write` scoped to exactly uplift/*/hubspot* (Put/Create/Describe; VERIFY-on-first-connect note kept); INTEGRATIONS_REAL_SECRETS gated behind var api_integrations_real (default false, API task only).
...
- **Status:** IN-PROGRESS (Nick) @feat/nick-req-006-008 — ci.yml gains the separate live-signup-e2e job (push-to-main + nightly cron, never PRs); secrets absent → every test self-skips → green. GH secrets STRIPE_TEST_* to be set when the user supplies test keys.
