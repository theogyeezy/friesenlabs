...
### REQ-012: Switchboard go-live — sync-runs migrate, connector-IAM deltas (Delete/List), the INTEGRATIONS/INGEST flips, module Price
- **Status:** OPEN
- **Requested by:** Lane Matt @feat/matt-switchboard-audit (PR "feat(switchboard): release readiness — disconnect, async sync runs, vault-derived schedule, verify-on-connect, secret purge, honest marketing")
- **Needed for:** releasing the Switchboard module ($29/mo `integration` in shared/modules.py) to paying customers. The code side is DONE and inert-safe: every new surface answers honest 503/no-op until the flips below. Full audit: `docs/audits/switchboard-audit-2026-06-11.md`.
- **Env/secret names** (already in shared/config.py — NO new names): `INTEGRATIONS_REAL_SECRETS` (REQ-008 gate `api_integrations_real`), `INGEST_REAL_STORES`, `INGEST_TENANTS` (new VALUE: the sentinel `auto`), `STRIPE_PRICE_ID_MODULE_INTEGRATION` (the `module_prices` tfvars map).
- **Spec (in apply order):**
  1. **Live migrate** — run the `uplift-migrate-oneoff` family with a fresh image: `db/schema.sql` adds the RLS-FORCEd tenant table `integration_sync_runs` (single-runner partial-unique guard + latest-idx) and `db/roles.sql` adds `GRANT SELECT, INSERT, UPDATE ON integration_sync_runs TO crm_app; REVOKE DELETE ...` (cross-lane mirror already in the file per the REQ-011 precedent — please review there). Isolation gate after, as always.
  2. **API task role — Secrets Manager DELETE** on exactly the connector slots (disconnect + the account-delete vault purge):
     ```hcl
     actions   = ["secretsmanager:DeleteSecret"]
     resources = [for src in ["hubspot", "stripe", "gohighlevel"] :
       "arn:aws:secretsmanager:${var.region}:${acct}:secret:${var.project}/*/${src}*"]
     ```
     (same resource shape as the REQ-008 `connector-write` sid; DeleteSecret is called with `ForceDeleteWithoutRecovery=true` — the value is the tenant's own re-pasteable token, and a recovery window would block reconnects.)
  3. **Ingest task role — `secretsmanager:ListSecrets`** (Resource `*`; list APIs are not resource-scopable, names/metadata only — no GetSecretValue change): powers `INGEST_TENANTS=auto`, which derives the nightly sync set from the vaulted `uplift/{tenant}/{source}` slots so a tenant who connects via the API is auto-enrolled (the connect→sync loop).
  4. **Verify REQ-008's widening is APPLIED live** — `infra/modules/iam/main.tf:253` now scopes connector-write to all three sources (#235), but the REQ-008 Status note still says hubspot-only; a targeted plan should show no diff if it's live.
  5. **Flip `api_integrations_real=true`** (INTEGRATIONS_REAL_SECRETS on the api task). First live connect is the standing `# VERIFY`: put/create/describe/delete_secret shapes + the verify-on-connect probe endpoints (HubSpot `GET /crm/v3/objects/contacts?limit=1`; GHL `GET /contacts/?limit=1` + `Version: 2021-07-28` — marked `# VERIFY` in `_PROBES`; Stripe `GET /v1/customers?limit=1`).
  6. **DECISION — API-kicked syncs:** "Sync now" needs `INGEST_REAL_STORES=1` on the API task (the runner + csv importer gates). REQ-004's old done-when kept INGEST_* off the api task because syncs ran IN-REQUEST; that rationale is now closed — syncs are backgrounded (202 + `integration_sync_runs` row) with a partial-unique single-runner guard and a 30-min stale-reap. Recommend flipping it WITH a deploy-invariance read of the other `INGEST_REAL_STORES` call-sites on the api image (`default_structured_sink`, csv import — both wanted live). If deferred, "Sync now" stays an honest 503 and only the schedule syncs.
  7. **Enable the schedule:** `ingest_tenants = "auto"` + `ingest_schedule_enabled = true`, targeted apply. NOTE the rule's command runs `--source hubspot` (the default) — either add per-source scheduled runs for `stripe`/`gohighlevel` now, or hold them until their first connected tenant; `auto` discovery already filters per source.
  8. **Module billing:** owner mints the $29/mo recurring Price for the `integration` module and sets `module_prices = { STRIPE_PRICE_ID_MODULE_INTEGRATION = "price_..." }` in tfvars (Phase-2 billing then carries it on enabled tenants' subscriptions).
- **Done when:** migrate exits 0 + isolation gate PASS; with `api_integrations_real=true` a live authed connect→`GET /integrations` shows `connected` with `verified:true`, `POST .../sync` answers 202 and the run row reaches `succeeded` (or stays an honest 503 if step 6 is deferred), DELETE credentials flips the status back and removes the secret (DescribeSecret NotFound); the nightly rule with `INGEST_TENANTS=auto` logs a discovered-tenant sync for a connected tenant with NO operator tfvars edit; account-delete on a connected test tenant reports `connector_secrets.status="purged"`; the module Price appears on a test tenant's subscription after an entitlement toggle.
- **Update (Lane Matt, authored):** steps 2, 3 and 6's terraform is now AUTHORED (PR "feat(infra): REQ-012 Switchboard go-live wiring"): connector-write gains DeleteSecret+GetSecretValue, the ingest role gains ListSecrets, and the new `api_ingest_real` var injects INGEST_REAL_STORES on the api task (default false). REQ-012 is now apply-only: tfvars flips + deploy.yml + the migrate one-off + the live VERIFYs.
- **Correction (Lane Matt):** step 8's tfvars map is named `stripe_module_price_ids` (not `module_prices`); the minted TEST-mode Price is `price_1ThHLBRCMItYjxIJAUlF0E1q` ($29/mo, matches the deployed test-mode plan prices) and the flips incl. this map are STAGED in the machine-local `infra/prod.auto.tfvars` — re-encode to `PROD_AUTO_TFVARS_B64` rides the deploy.
- **EXECUTED (2026-06-12, owner-approved pipeline + Migrate workflow):** steps 1-4 and 6-8 are DONE LIVE — deploy run 27394841845 (owner-approved) applied the IAM deltas + all four flips + the module Price (`uplift-api:414e82c` rolled, healthz 200); Migrate run 27395832294 registered `uplift-migrate-oneoff:5` and ran `api.migrate` (exit 0, "schema + roles loaded") + the isolation gate (exit 0, "[isolation] PASS — RLS enforced"); unauthed `/api/integrations` + `/api/integrations/{name}/syncs` answer 401 live (mounted + gated). REMAINING: step 5's first-connect live # VERIFY (needs a real HubSpot/Stripe token pasted in the panel — probe + vault shapes + the GHL probe endpoint), and the nightly rule's first `auto` run log. Nick: please flip the Status line when you've reviewed the applied plan.

---

### REQ-013: Security-hardening batch (audit remediation) — deploy-role scope-down, Cognito hardening, env arming, flow/WAF/exec logging, cube SG split, ECS hardening, Aurora CMK path, RBAC groups
- **Status:** MOSTLY APPLIED — LIVE 2026-06-12 (owner-directed). Unconditional items (scoped deploy policy, `ALLOW_ADMIN_USER_PASSWORD_AUTH` removed, `UPLIFT_ENVIRONMENT=prod`, Cognito threat-protection ENFORCED + admin/member groups, VPC flow logs `/vpc/uplift-flow-logs` ACTIVE, WAF logging → `aws-waf-logs-uplift-api-edge`, ECS-exec OVERRIDE logging → `/ecs/uplift-exec`) rode the owner-approved `deploy.yml` applies. Then two gated flips were applied as targeted, plan-reviewed steps: **(7) `deploy_role_admin_fallback=false`** — `AdministratorAccess` DETACHED, `uplift-deploy` now holds only `uplift-deploy-scoped` (proven by the deploy run that built `uplift-api:2da9037`); **(8) `worker_dedicated_sg=true` + `provisioning_lambda_dedicated_sg=true`** — worker + provisioning Lambda moved to dedicated SGs, the sg_api self-:4000 rule removed; cube:4000 now admits only the api + worker SGs (verified), worker re-rolled and healthy (polling 200, cube/db reachable via explicit rules). **Final baseline plan: CLEAN (No changes).** **(RBAC strict, 2026-06-12)** `RBAC_STRICT` was plumbed onto the API task (PR #315, gated `rbac_strict` var) and flipped live: the tenant-bearing user `matthew.samuel.yee@gmail.com` was added to the `admin` Cognito group first (it is the only pool account carrying a `custom:tenant_id` claim — the other 4 can't authenticate, so no lockout), then `rbac_strict=true` → targeted `module.api_service` apply (1 add task-def `uplift-api:28` + 1 in-place service, image unchanged), rollout COMPLETED, `RBAC_STRICT=1` verified on the live task, edge healthz 200, `module.api_service` re-plan CLEAN. The empty-`cognito:groups`=admin back-compat is now off. DEFERRED by design: Aurora CMK (item 9 — planned maintenance window only, runbook below), `adot_image` digest pin (8a — owner tfvars), `readonly_root_filesystem` (8b — verify window). All flips are in the machine-local `infra/prod.auto.tfvars`.
- **Requested by:** Lane Matt @feat/matt-security-audit (security-audit remediation batch; infra-author exception — all edits are `infra/**` + this file + RUNBOOK)
- **Needed for:** closing the audit's P0s (admin OIDC deploy role, public-client admin auth flow, dead `is_prod()` guard) + the logging/SG/encryption hardening items.
- **Env/secret names:** `UPLIFT_ENVIRONMENT` (already read by `shared/config.py`). No new secrets.
- **Design rule honored:** P0 fixes are UNCONDITIONAL; everything that rolls tasks/replaces resources is gated on a variable whose default = the CURRENT LIVE state (zero-diff by default). The baseline plan should therefore show only the unconditional adds/changes listed below.

| # | Item | Gated? (flip var, default) | Plan impact at defaults |
|---|------|---------------------------|--------------------------|
| 1 | Scoped deploy policy `uplift-deploy-scoped` attached to `uplift-deploy` | detach-admin gated: `deploy_role_admin_fallback` (**true** = admin stays) | +1 policy, +1 attachment; admin attachment unchanged (`moved` block keeps its address) |
| 2 | `ALLOW_ADMIN_USER_PASSWORD_AUTH` REMOVED from the public SPA client | optional replacement client: `create_smoke_test_client` (**false**) | 1 in-place client update (P0, unconditional) |
| 3 | `UPLIFT_ENVIRONMENT=prod` on the api task + provisioning Lambda | value via `uplift_environment` (**"prod"**) | new api task-def revision (service rolls, zero-downtime) + Lambda env update (P0, unconditional) |
| 4 | Cognito threat protection `user_pool_add_ons.advanced_security_mode` | mode via `cognito_threat_protection_mode` (**"ENFORCED"**; "AUDIT" = observe-only rollback); tier via `cognito_user_pool_tier` (**""** = unmanaged) | 1 in-place pool update — ⚠️ NEEDS the Plus feature plan: if the apply errors on the feature plan, set `cognito_user_pool_tier="PLUS"` (billing change, ~per-MAU) in the SAME apply, or temporarily use "AUDIT"/"OFF" |
| 5 | VPC flow logs (ALL) → `/vpc/uplift-flow-logs` + delivery role | no (unconditional, additive) | +4 resources |
| 6 | WAF logging → `aws-waf-logs-uplift-api-edge` (us-east-1) | no (unconditional, additive) | +2 resources |
| 7 | Cube SG split: new `uplift-cube` SG, cube service moves to it; self-:4000 rule on sg_api REMOVED | free-rider severing gated: `worker_dedicated_sg` (**false**), `provisioning_lambda_dedicated_sg` (**false**) | +4 (SG, 2 rules, db pairing), -1 rule, cube service network update — ⚠️ ROLLS CUBE TASKS (brief cube unavailability while tasks re-register in Cloud Map; api/worker degrade visibly, no data loss) |
| 8a | `adot_image` threaded to api/cube/worker sidecars | pin via `adot_image` (default = exact live `:latest` string) | zero-diff; **ACTION: pin a digest** (`public.ecr.aws/aws-observability/aws-otel-collector@sha256:…`) in tfvars — rolls all three services when changed |
| 8b | `readonlyRootFilesystem` + /tmp ephemeral volume on app containers | `readonly_root_filesystem` (**false**) | zero-diff; flip rolls api+cube+worker — flip in a verify window (cube is a third-party image; circuit breakers auto-roll-back) |
| 8c | ECS Exec audit: cluster `execute_command_configuration` (OVERRIDE → KMS-encrypted `/ecs/uplift-exec`) + api-task kms/logs grants; `enable_ecs_exec` | exec on/off via `enable_ecs_exec` (**true** = live state) | +3 (KMS key, alias, log group), cluster in-place update, +1 task-role policy. NOTE: after apply, `aws ecs execute-command` callers need `kms:Decrypt` on `alias/uplift-ecs-exec` (admin sessions have it via the key policy root delegation) |
| 9 | Aurora CMK path | `aurora_kms_key_arn` (**""**), `create_aurora_cmk` (**false**) | zero-diff. ⚠️⚠️ Setting either REPLACES THE LIVE CLUSTER — see the migration runbook below. |
| 10 | Cognito user groups `admin`/`member` + `AdminAddUserToGroup` on the api-task + provisioning-Lambda cognito policies | no (unconditional, additive) | +2 groups, 2 in-place policy updates |
| 11 | X-Origin-Verify rotation runbook | n/a (docs) | none — see `infra/RUNBOOK.md` |

- **Apply order (each step its own targeted apply + verify):**
  1. `-target=module.iam` → items 1, 8c-grants, 10-policy. Verify: `aws iam list-attached-role-policies --role-name uplift-deploy` shows BOTH `uplift-deploy-scoped` and `AdministratorAccess`.
  2. `-target=module.ecs` → item 8c (KMS key + exec log group + cluster config; in-place). Verify: `aws ecs describe-clusters --include CONFIGURATIONS` shows executeCommandConfiguration logging=OVERRIDE.
  3. `-target=module.vpc -target=module.api_cdn` → items 5 + 6 (pure adds). Verify: flow-log ACTIVE on the VPC; `aws wafv2 get-logging-configuration --scope CLOUDFRONT` returns the log-group ARN; both log groups exist with retention 30.
  4. `-target=module.auth` → items 2, 4, 10-groups. ⚠️ If the apply errors with a feature-plan message, set `cognito_user_pool_tier="PLUS"` and re-apply (or drop to "AUDIT" / "OFF" while deciding on the Plus cost). Verify: `describe-user-pool` shows AdvancedSecurityMode + the 2 groups; `describe-user-pool-client` (web) shows NO ADMIN_USER_PASSWORD_AUTH. **Smoke-test note:** any smoke script using admin-initiate-auth against the web client breaks at this point — either migrate it to the gated smoke client (`create_smoke_test_client=true`, confidential, needs SECRET_HASH) or to the Hosted-UI flow.
  5. `-target=module.security -target=module.cube` → item 7. ⚠️ Rolls cube tasks. Verify: cube `/readyz` 200 from a one-off task in sg_api; `aws ec2 describe-security-groups` shows uplift-cube with the single :4000 ingress from sg_api and NO :4000 rule left on sg_api; worker `query_cube` still returns rows (worker shares sg_api until `worker_dedicated_sg` flips).
  6. `-target=module.api_service -target=module.provisioning_lambda` → item 3 (+ the threaded vars). Rolls the api service (zero-downtime, circuit breaker armed). Verify: new task def shows `UPLIFT_ENVIRONMENT=prod`; edge `/healthz` 200; Lambda env shows the var.
  7. **Deploy-role flip (item 1, the migration path):** after steps 1–6 are green, run ONE full `deploy.yml` cycle end-to-end (build → plan → apply → service roll → health check) with admin still attached. If it passes with zero AccessDenied in CloudTrail for `uplift-deploy`, set `deploy_role_admin_fallback=false` + `-target=module.iam` apply → admin detaches. Watch the NEXT deploy closely; rollback = set true + re-apply.
  8. **Later flips (deliberate, separate):** `worker_dedicated_sg=true` (+ worker roll; verify worker query_cube + DB reads), `provisioning_lambda_dedicated_sg=true` (verify a stub SFN invocation reaches Aurora), `readonly_root_filesystem=true` (verify window), digest-pin `adot_image`.

- **Aurora CMK migration runbook (item 9 — execute ONLY in a planned maintenance window):**
  1. Create the key first WITHOUT wiring: `create_aurora_cmk=true` would wire it — so instead apply ONLY the key via `-target=module.data.aws_kms_key.aurora -target=module.data.aws_kms_alias.aurora` after setting the flag, and DO NOT run a broader apply yet (the un-targeted plan now shows the cluster replacement — that is the signal you are mid-migration).
  2. Quiesce writes (scale api to 0 or maintenance-page), take a manual snapshot: `aws rds create-db-cluster-snapshot`.
  3. `aws rds copy-db-cluster-snapshot --kms-key-id alias/uplift-aurora` (re-encrypts under the CMK).
  4. Restore: `aws rds restore-db-cluster-from-snapshot` (+ create the serverless instance, same subnet group/SG/params), verify row counts + the RLS isolation gate against the restored endpoint.
  5. Cut over: rename clusters (or update DB_HOST wiring) so the restored cluster owns `uplift-aurora`'s identity; `terraform state rm` the old cluster+instance, `terraform import` the new ones; full plan must come back clean with kms_key_id + performance_insights_kms_key_id now matching the CMK.
  6. Keep the old cluster stopped for the rollback window, then delete (final snapshot retained).
  Until this window, NEVER set `aurora_kms_key_arn`/`create_aurora_cmk` in tfvars and apply blind — the plan will propose destroying the live tenant database.

- **Done when:** every numbered verify above passes; the baseline plan at default flips is clean; `uplift-deploy` runs deploys without AdministratorAccess (item 1 flipped false); REQUESTS.md status updated by Nick.
