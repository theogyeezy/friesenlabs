...
- **Status:** IN-PROGRESS (Nick) — GRANT + explicit `REVOKE DELETE` (reconciling the default-privileges note) authored in `db/roles.sql` @feat/nick-req-001-002; live GRANT lands via the next one-off migrate/roles task against Aurora
