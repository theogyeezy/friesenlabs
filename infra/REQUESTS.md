...
- **Status:** IN-PROGRESS (Nick) @feat/nick-req-004 — new `infra/modules/ingest`: dedicated task def (arm64 api image, command override, INGEST_REAL_STORES=1 on this def only) + dedicated task role (per-tenant `uplift/*/hubspot-*` + deprecated shared token + `bedrock:InvokeModel` on `amazon.titan-embed-text-v2:0` exactly as `ingest/embed.py` calls it + conditional raw-bucket PutObject) + EventBridge rate(1 day) rule DISABLED by default with a scoped events role (RunTask cluster-conditioned + PassRole service-conditioned). All pure adds at safe defaults; batch-backfill IAM deferred to its own REQ per the spec.
