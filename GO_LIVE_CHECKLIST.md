...
- [x] `playbook_dispatch_enabled = true` + `playbook_dispatch_tenants = "<tenant-id,...>"` → ONE flag now does both legs (#289): it enables the EventBridge dispatcher AND stamps `PLAYBOOK_DISPATCH_ENABLED=1` on the api task, so the Studio's "trigger not enabled yet" banner clears in the same apply. **DONE 2026-06-12 (owner-approved deploys 27402480660 + 27403183085): rule ENABLED on `cron(0/15 * * * ? *)` (quarter-hour-ALIGNED — #296; `rate(15m)` ticked at :12/:27/:42/:57 and would never match the minute-0 starter-template crons), tenant list = the demo tenant, api task rev 17 carries the env, edge healthz 200, and the 08:15Z tick VERIFIED in `/ecs/uplift-playbook-dispatch`: "dispatch complete: 0 playbook run(s) across 1 tenant(s)". Playbook crons must use minutes 0/15/30/45 (dispatcher exact-minute match; window-matching is a tracked follow-up). New tenants must be appended to `playbook_dispatch_tenants` until DB discovery ships (tracked P1). Side quests this flip surfaced + fixed: #294 (#290's ECS-exec audit `count` on apply-time ARNs broke every plan — and its first failed apply deregistered the api task def, restored by the next apply), #295 (deploy retries on an unchanged SHA died on the immutable ECR tag — build now skips when the image exists), and `cognito_threat_protection_mode = "OFF"` pinned in tfvars (#290's ENFORCED default 400s on the live ESSENTIALS pool tier — enabling it is a billing decision: flip tier to PLUS + mode together).** **FULL-RUN PROOF + two follow-ons 2026-06-12 (deploy 27424293638): (a) a SECOND firing bug found live — Fargate starts ~30-90s after the tick, so the dispatcher's `datetime.now()` minute missed every cron; fixed by #299 (`_tick_floor` to the quarter-hour). Then an ACTIVE `*/15` playbook (pipeline-hygiene-scout, demo tenant) FIRED on the 15:15Z tick: real MA session → "dispatch complete: 1 playbook run(s) across 1 tenant(s)" → run persisted (`schedule · */15 · pending`) → 2 draft actions in Greenlight, 0 approved (draft-only HELD on a scheduled run) → `reused_registration: true` (no crew leak). Playbook deactivated after the proof. (b) Cognito flipped to `cognito_user_pool_tier = "PLUS"` + `cognito_threat_protection_mode = "ENFORCED"` in the same apply — owner-approved billing change, verified live (describe-user-pool: PLUS/ENFORCED). REQ-012 items 2+4 CLOSED.**
...
- [x] **One-off DB migrate** for `playbook_runs` + the `playbooks.ma_*` columns — **DONE 2026-06-12 (migrate.yml run succeeded 05:07Z; idempotent schema.sql + roles.sql batch).**
