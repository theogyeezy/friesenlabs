...
def test_onboarding_state_round_trip_and_isolation():
    dsn = _app_dsn()
    store = OnboardingStateStore(dsn)
...
def test_crm_app_cannot_delete_onboarding_state():
    ...
    OnboardingStateStore(dsn).upsert(tenant, steps={"load_data": True})
    conn = psycopg2.connect(dsn)
    try:
        with conn.cursor() as cur:
            cur.execute("SET app.current_tenant = %s", (tenant,))
...
@pytest.mark.integration
def test_load_sample_is_idempotent_into_the_calling_tenant():
    dsn = _app_dsn()
    store = OnboardingStateStore(dsn)
    ...
    counts1 = _load_sample_into_tenant(store, tenant)
    counts2 = _load_sample_into_tenant(store, tenant)
    assert counts1["companies"] == counts2["companies"] == 40
    assert counts2["contacts"] == 120 and counts2["deals"] == 60

    got = _crm_counts(dsn, tenant)
    assert got["companies"] == 40, "fixture loaded once, not duplicated on re-run"
    assert got["contacts"] == 120 and got["deals"] == 60
