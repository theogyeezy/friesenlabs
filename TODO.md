...
### P0 — before paying customers
- [ ] **Approval expiry** — schema names an `expired` status (`db/schema.sql:99`) but nothing
...
- [ ] **Paginate `GET /approvals`** (`api/app.py:234-236`) — returns the bare unbounded
  `list_pending` list; add limit+cursor (reuse the traces keyset pattern), default 100.
- [ ] **Index `approvals (tenant_id, status, created_at)`** — no index exists on `approvals`;
  `list_pending` scans. Author migration (Lane Matt); live apply is `BLOCKED: Lane Nick`.
- [ ] **Approvals discoverability in real mode** — nav badge is demo-only
...
- [ ] **Show what's being approved** — draft text falls back to `""` when `proposed_action`
...
- [ ] **Isolation gate: probe `approvals`** — `scripts/isolation_test.py` never touches the
  approvals table (documents+FK only; traces covered in `test_control_rls.py`). Add a
  cross-tenant approval read/decide probe.

### P1 — soon after
- [ ] **Kill switch on the chat path** — no kill-switch check in `conv/session.py` or
...
- [ ] **Stop draft-only reading as a real send** — appliers return `performed:false`
...
- [ ] **Already-decided UX** — a 400 "not pending" (another user won) shows the generic
...
- [ ] **`GreenlightQueue` 404 degrade parity** — give it the same "not yet enabled"
  feature-detection SecurityControls has (`web/src/api/SecurityControls.tsx`) instead of a
  generic error.
- [ ] **Registry/roster drift** — `worker.TOOLS` is hand-curated (`worker/worker.py:56-60`);
...
- [ ] **Applier failure handling** — record more than `e.__class__.__name__`
...
- [ ] **Deny reason** — real UI hardcodes "Declined by reviewer."
...
- [ ] **Compliance depth** — quiet hours hardcoded 21:00–08:00 trusting payload-supplied
  `local_hour` (no tenant timezone source); compliance blocks aren't logged.

### P2 — hygiene
- [ ] Document (or role-gate later) that any tenant member can approve anything and flip the
  tenant kill switch/dial — v1 "every authed user is admin" is a deliberate posture, say so.
- [ ] Retention/archival policy for decided approvals (append-forever today).
- [ ] Test gaps: pool-exhaustion retry (`api/control/greenlight.py:115-130`), dial TTL-cache
  read-your-own-write, e2e deny flow, e2e double-decide/stale-approval.
- [ ] Thread `approval_id`/`decided_by` into the applier signature for downstream audit/Cortex
  feedback tie-in.
