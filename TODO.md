...
- [x] **Seed the knowledge corpus (demo tenant)** — SEEDED 2026-06-12: 26 docs / 26 chunks (Titan V2) via a `uplift-migrate-oneoff` one-off on the live api image (`PYTHONPATH=/app` workaround — bootstrap now fixed); retrieval verified in-VPC: top hit `demo:kb:pricing-discount-authority#0` @0.487, RLS-scoped, demo inventory 195 upload docs.
...
- [x] **Web-UI wave (#228 client methods + #229 views):** the REAL authed app had no Cortex/billing/import tabs (the backlog items pointed at the demo-only `screens/*` mock prototype). Built net-new real-mode views in `web/src/api/*`: **CortexView** (`GET /cortex/health`, honest states, NO number simulation), **CSV upload** (IntegrationsPanel → `POST /integrations/csv/import`), **invoice display** (BillingManage → `GET /billing/invoices`), **account export/delete** (Settings). All render honest-empty/degraded until the owner-gated prod flips. _(The demo-prototype `screens/cortex.tsx` number-simulation is intentionally left — it's mock-mode-only, part of the deferred demo-honesty work.)_
- [x] **Depth wave (#231/#232/#233):** **CRM-table landing** (CSV imports now land in the Pipeline/Contacts tables via `default_structured_sink` — the ref→uuid `PgCrmStructuredSink` already existed; #231); **status page real per-subsystem probes** (`GET /public/status` with the "unknown never degrades the rollup" fix + sanitized public error detail; web `fetchStatus` reads it; #232/#233); **settings persistence** (`GET/PUT /account/settings` over new `tenant_settings` columns + a real-mode Workspace settings UI that saves; #232/#233); **Agent marketplace real-mode** (the backend `/studio/templates` already existed — built the `MarketplaceView` browse+hire over it; #233). All ship honest-by-default (503/degrade) until the owner-gated prod wiring: settings column migrate, real status probes in `asgi.py`, the ingest-env flip for live CRM-table landing. _(account-delete deliberately kept inert — owner decision.)_
...
- [x] **Knowledge corpus seed (scripts/demo/seed_knowledge.py + agents/knowledge_seed)** — DONE: SEEDED 2026-06-12: 26 docs / 26 chunks (Titan V2) via a `uplift-migrate-oneoff` one-off on the live api image (`PYTHONPATH=/app` workaround — bootstrap now fixed); retrieval verified in-VPC: top hit `demo:kb:pricing-discount-authority#0` @0.487, RLS-scoped, demo inventory 195 upload docs.
...
| ✅ **Live & verified** | AI / agent plane (chat, tool execution) | `scripts/verify_agent_plane.py` PASSED live 2026-06-10: provision 7 agents + coordinator → `/chat` answer + delegation → Greenlight approve/execute, **draft-only held**. Worker **2/2 polling**. Bedrock-embed IAM gap caught + fixed | — (demo corpus SEEDED 2026-06-12 — retrieval verified; customers self-populate via Knowledge → Add document since #251) |
...
**Implementation batch shipped same day (2026-06-11, Lane Matt, this branch):** all four P0s, the
P1/P2 code items, and the full terraform authoring (REQ-013) — 5 parallel implementers +
2-stage review; full suite **2104 passed / 0 failed**, web typecheck+build green,
...
- [x] **[HIGH] Set `UPLIFT_ENVIRONMENT=prod` on the API + provisioning-Lambda tasks** **AUTHORED 2026-06-11 (both task envs; root var default "prod"). Apply = Lane Nick (api task-def roll).** — `shared/config.py:228-245` `is_prod()` reads it but no task sets it, so the refuse-to-boot-with-Stripe-bypass-in-prod guard is dead code; only the empty default keeps comped provisioning off. _(infra env wire; apply Lane Nick)_
- [x] **[HIGH→latent] Run compliance (TCPA/CAN-SPAM) in `Greenlight.propose` + re-validate after human `edit`** **DONE 2026-06-11: compliance floor inside `propose` (trusted-registry channel resolution, unknown action fail-closed, violations stored denied/non-executable) + post-edit re-validation before the CAS flip; 4 view/dashboard 422s no longer echo `str(e)`. Review add-on: `POST /approvals/{id}/decide` admin-gated.** — `api/control/compliance.py:41` has ONE call site (`api/control/gate.py:44`); worker (`agents/tools/base.py:94`), Sidecar (`api/sidecar_routes.py:94`), playbooks, and `apply_approved_action` (`api/app.py:283-284`) all skip it. Masked only by `record_only` appliers — must land BEFORE any real sender is wired into `APPLIERS`. _(Lane Matt, code)_

### P1 — high-value hardening (before/at first team-tenant)
- [x] **[HIGH] Intra-tenant RBAC** **DONE 2026-06-11 (code): `cognito:groups` → Claims; ONE admin policy (empty-groups back-compat, `RBAC_STRICT=1` retires it); 8 writes admin-gated (killswitch/autonomy/portal/modules/export/delete/settings/approval-decide), reads open; global killswitch operators now user-granular `CONTROL_GLOBAL_OPERATOR_USERS` (fail-closed); provisioning bootstraps first user into "admin" best-effort. TF authored: admin/member groups + AdminAddUserToGroup on both pool policies. Remaining flips (Nick/owner): apply groups, set operator env, then `RBAC_STRICT=1`.** — any authed tenant user can flip the kill switch, set autonomy L3, open the billing portal, change paid module entitlements, trigger GDPR export (`api/routes_control.py:17-30`, `api/billing_routes.py:59-96`, `api/modules_routes.py:88-109`). Acceptable solo-tenant; not for teams. Add a Cognito group/role claim + gate the privileged routes. Also make global kill-switch operators user-granular, not tenant-granular (`routes_control.py:66-69` — today every user of an operator tenant can pause the whole platform). _(Lane Matt code + pool attribute via Nick)_
- [x] **[HIGH] Cognito threat protection (advanced security ENFORCED)** **AUTHORED 2026-06-11 (`user_pool_add_ons.advanced_security_mode`, var default ENFORCED — verified current for provider 6.49; needs the PLUS tier, cost note in REQ-013). Apply = Lane Nick.** — no `user_pool_add_ons` in `infra/modules/auth/main.tf`; verify the `~> 6.49` provider attribute name first. _(apply Lane Nick)_
- [x] **[HIGH] VPC flow logs** **AUTHORED 2026-06-11 (traffic_type ALL → CW group on `log_retention_days` + delivery role; unconditional). Apply = Lane Nick.** — no `aws_flow_log` in any module; GuardDuty has no flow signal. _(apply Lane Nick)_
- [x] **[HIGH] WAF logging** **AUTHORED 2026-06-11 (`aws-waf-logs-uplift-api-edge` CW group + logging config on the CLOUDFRONT ACL; unconditional). Apply = Lane Nick.** — WAFv2 ACL has managed rules + rate limit but no `aws_wafv2_web_acl_logging_configuration` (`infra/modules/api_cdn/main.tf:27-97`); no edge forensics. _(apply Lane Nick)_
- [x] **[MED] SPA security headers + CSP on the Amplify origin** **DONE 2026-06-11: `customHttp.yml` at repo root (monorepo appRoot form) — HSTS/nosniff/Referrer-Policy/XFO DENY/Permissions-Policy + code-derived CSP (Cognito, Turnstile, Google Fonts; PostHog same-origin). `'unsafe-eval'` kept ONLY for vega (loud comment) — follow-up: wire `vega-interpreter` and drop it. VERIFY on an Amplify preview before trusting (headers land, charts render, Hosted-UI round-trips).** — `infra/modules/web_hosting/main.tf` sets none (the headers policy at `api_cdn/main.tf:141-161` covers only the API distro); refresh token sits in localStorage with DOMPurify as the sole XSS defense. Amplify `custom_headers`/`customHttp.yml`: CSP, `frame-ancestors 'none'`, nosniff, referrer-policy, HSTS. Consider refresh-token-out-of-localStorage as a follow-up. _(author Lane Matt; deploy via Amplify)_
- [ ] **[MED] Aurora + Performance Insights onto a customer-managed KMS key** _(AUTHORED 2026-06-11 behind `aurora_kms_key_arn`/`create_aurora_cmk` — defaults off, zero-diff; descriptions + REQ-013 scream the cluster-REPLACEMENT hazard and carry the snapshot-restore runbook. The live migration itself stays open = Lane Nick, runbook'd window.)_ — `infra/modules/data/main.tf:25,56`; PI captures query text (tenant PII in literals). ⚠️ `kms_key_id` change = cluster REPLACEMENT (snapshot-restore migration; never let a plan propose it silently). _(plan + apply Lane Nick, runbook'd)_
...
- [x] **[MED] Pg-backed used-token store for email verification** **DONE 2026-06-11: `PgUsedTokenStore` over `accounts.meta` jsonb (atomic prune-on-write merge, mirrors PgOtpStore — no new table), wired in prod_deps; cross-task replay closed.** — `api/prod_deps.py:591-595` defaults to in-memory per task → cross-task replay within TTL (OTP path already uses `PgOtpStore`; mirror it). _(Lane Matt)_

### P2 — defense-in-depth
- [x] **[MED] Whitelist Vega chart `spec` fragment keys** **DONE 2026-06-11: {mark, encoding, transform} allow-list + recursive href/url rejection enforced in shared/view_spec.py + the JSON schema (server = enforcement) and mirrored in viewSpec.ts; SpecRenderer defensively strips unknown keys render-inert.** — `web/src/dashboard/viewSpec.ts:351-360` allows any object, spread into vega-embed (`SpecRenderer.tsx:297-304`); containment currently rests on vega-embed config (SVG, no actions, loader off). Allow `mark`/`encoding`/vetted `transform`; reject `params`/`signals`/`href`/`usermeta`/`data`; mirror in `shared/schemas/view_spec.schema.json`; pin vega versions. _(Lane Matt)_
- [x] **[MED] Delimit untrusted tenant content in agent prompts** **DONE 2026-06-11: UNTRUSTED-DATA fences + treat-as-data preamble around RAG snippets (synthesizer) and playbook trigger payloads (runner); citation contract unchanged.** — RAG chunks (`conv/synthesizer.py:170-177`), tool results (`agents/runtime.py:434-460`), playbook trigger payloads (`agents/playbooks/runner.py:125-126`, reachable from `/public/leads` content) are concatenated raw; injection can steer drafts a human rubber-stamps. Wrap in treat-as-data delimiters. _(Lane Matt)_
- [x] **[MED] ECS hardening batch** **AUTHORED 2026-06-11: `adot_image` var (default = current `:latest` string; digest pin = Nick tfvars), `readonly_root_filesystem` (default off; flip adds /tmp ephemeral volume), `enable_ecs_exec` (default on = live) + cluster exec session logging → KMS'd CW group (unconditional). Flips = Lane Nick.** — pin the ADOT sidecar by digest (floats `:latest` on api/cube/worker); add `readonlyRootFilesystem` + non-root `user`; gate `enable_execute_command` behind a var + add an `execute_command_configuration` session-log sink. _(apply Lane Nick)_
- [x] **[MED] Split cube out of the shared `sg_api`** **AUTHORED 2026-06-11: dedicated `uplift-cube` SG (:4000 from sg_api only, + db pairing), sg_api self-rule removed; gated `worker_dedicated_sg`/`provisioning_lambda_dedicated_sg` (default off) for full tier isolation — worker keeps its legitimate cube reach. Apply = Lane Nick (rolls cube tasks).** — `infra/modules/security/main.tf:116-124` self-rule gives worker/provisioning-Lambda free reach to cube:4000 (+Aurora:5432); replace with an explicit api→cube rule. _(apply Lane Nick)_
- [x] **[LOW] Mask PII in send logs** **DONE 2026-06-11 (`mask_email`/`mask_phone` helpers; all 6 log/error sites masked; payloads keep real addresses).** — full email (`signup/resend_sender.py:128,149,152`), full phone (`signup/sms_sender.py:59,82`). _(Lane Matt)_
- [x] **[LOW] Worker: assert `ANTHROPIC_API_KEY` is ABSENT at startup** **DONE 2026-06-11 (fail-loud RuntimeError at `build_clients_from_env`; `WORKER_ALLOW_ORG_KEY=1` dev-only override, exact-match).** — `worker/worker.py:203-206` happily builds an org-key client in the creds-laden container if it's ever misconfigured onto the task. _(Lane Matt)_
- [x] **[LOW] Replace the raw `innerHTML` sinks on the landing constellation** **DONE 2026-06-11 (segment arrays + createElement/textContent; zero innerHTML remains; identical DOM).** — `web/src/screens/landing-constellation.tsx:228,238` (static constants today, violates the SafeHtml-only rule). _(Lane Matt)_
- [x] **[LOW] Tighten JWT `token_use` to require `"id"`** **DONE 2026-06-11** (`api/auth.py:65-66`) **+ stop echoing `str(e)` in 422s** (`api/app.py:351,363,396,431`, `api/signup_routes.py:268,276`). _(Lane Matt)_
- [x] **[LOW] X-Origin-Verify secret rotation runbook** **DONE 2026-06-11 (RUNBOOK.md: SAFE 3-phase no-403 path + FAST single-window path).** — generated once, never rotated (`infra/modules/secrets/main.tf:49-64`); document regenerate→CloudFront header→ALB rule. _(Lane Nick, runbook)_
